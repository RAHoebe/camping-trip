(function () {
  ...
  data.stops.forEach(function (stop, index) {
    ...
    L.marker(latlng, { icon: stopIcon(index + 1) })
      .bindPopup(`<strong>${escapeHtml(stop.name)}</strong><br>${escapeHtml(stop.arrival_date || "")}${stop.departure_date ? " to " + escapeHtml(stop.departure_date) : ""}`)
      .addTo(map);
  });
  ...
  if (data.admin) {
    const campgroundLayer = L.layerGroup().addTo(map);
    ...
    function loadVisibleCampgrounds() {
      if (!data.admin.campgroundsSearchUrl || map.getZoom() < 8) {
        campgroundLayer.clearLayers();
        return;
      }
    ...
    function searchCampgroundsAroundCenter() {
      if (!data.admin.campgroundsSearchUrl) return;
    ...
    const SearchControl = L.Control.extend({
      ...
      onAdd: function () {
        const container = L.DomUtil.create("div", "leaflet-bar campground-search-control");
        const button = L.DomUtil.create("button", "", container);
        button.type = "button";
        button.title = "Search GpxFeed campsites within 20 km of the map center";
        button.innerHTML = '<i class="bi bi-search"></i><span>20 km</span>';
  ...
  fetch(`/api/trips/${data.tripId}/route`)
    .then(function (response) {
    ...
    .then(function (routeData) {
      if (routeData.route && routeData.route.coordinates) {
        ...
        setStatus(`Route by ${routeData.provider}${routeData.distance_m ? " - " + formatDistance(routeData.distance_m) : ""}`, "ok");
      } else if (routeData.status === "not_enough_stops") {
        setStatus(routeData.message || "Add at least two campsites to calculate a route.", "muted");
      } else {
        setStatus(routeData.message ? `Route unavailable: ${routeData.message}` : "Route unavailable.", "warning");
      }
    })
    .catch(function (error) {
